...
export function getQueryString(key: string, default_: string = ""): string {
...
export function can(o: any,n: string){
  return n in o && typeof o[n]==="function" && o[n];
}
export const timeout=(t:number):Promise<void>=>new Promise(s=>setTimeout(s,t));
export function mutablePromise<T=void>(): MutablePromise<T> {
    return new MutablePromise<T>();
}
...
export async function _confirm(p: string): Promise<boolean> {
 ...
 * @returns string
 */
export function getEnv(n: string): string {
...
export function blob2arrayBuffer(blob: Blob): Promise<ArrayBuffer> {
  const r=mutablePromise<ArrayBuffer>();
