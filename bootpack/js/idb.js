    ...
    static open(dbName, stores = []) {
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = () => {
                ...
                const missing = requestedStores.filter(name => !db.objectStoreNames.contains(name));
                if (missing.length === 0) {
                    resolve(new Idb(db));
                    return;
                }
                ...
                upgradeReq.onblocked = () => {
                    reject(new Error(`IndexedDB upgrade blocked: ${dbName}`));
                };
            };
            req.onerror = () => {
                reject(req.error);
            ...
            req.onblocked = () => {
                reject(new Error(`IndexedDB open blocked: ${dbName}`));
            };
        });
    }
    /**
